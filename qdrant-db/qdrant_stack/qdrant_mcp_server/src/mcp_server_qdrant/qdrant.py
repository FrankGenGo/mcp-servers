...
class QdrantConnector:
    ...
    def __init__(
        ...
        logger.info(f"QdrantConnector initialized with URL: {self._qdrant_url or qdrant_local_path}")
    ...
    async def store(self, entry: Entry):
        ...
        logger.info(f"Document stored successfully in collection {self._collection_name}")
    ...
    async def search(self, query: str) -> List[Entry]:
        ...
        # For compatibility with the reference implementation, use the same search method
        # Even though it's deprecated, it ensures our tests work correctly
        ...
        logger.info(f"Found {len(entries)} documents matching query: {query}")
        return entries